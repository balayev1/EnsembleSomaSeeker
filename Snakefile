...
def get_resource_filter(wildcards):
    """Dynamically builds exclusion commands. Resources should be pre-normalized."""
    cmd = ""
    # Use -T ^ to exclude sites found in these files
    if config['mutect2_params'].get('panel_of_normals'):
        cmd += f" | bcftools view -T ^ {config['mutect2_params'].get('panel_of_normals')}"
    
    if config.get("hc_pon"):
        cmd += f" | bcftools view -T ^ {config['hc_pon']}"
    
    if wildcards.type == "SNV" and config.get("germline_snv_resource"):
        cmd += f" | bcftools view -T ^ {config['germline_snv_resource']}"
            
    elif wildcards.type == "INDEL" and config.get("germline_indel_resource"):
        cmd += f" | bcftools view -T ^ {config['germline_indel_resource']}"
